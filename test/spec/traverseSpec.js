...
import { expect, createModdle, readModdle } from '../helper';

describe('traverse', function() {
  it('should traverse each node of a diagram with multiple nodes', async function() {
    // given
    const { root: moddleWithMultipleDef } = await readModdle(__dirname + '/diagram.bpmn');
    let nodesCount = 0;
  ...
  it('should traverse each node of a diagram with one node', async function() {
    ...
    const { root: moddleWithOneDef } = await createModdle(xmlStr);
    let nodesCount = 0;
    const traverseCb = () => nodesCount++;
