...
function verifyOnly(ruleName, rule, testCases) {
  const {
    valid,
    invalid
  } = testCases;

  verify(ruleName, rule, {
    valid: valid.map(testCase => ({ ...testCase, it: it.only })),
    invalid: invalid.map(testCase => ({ ...testCase, it: it.only }))
  });
}
