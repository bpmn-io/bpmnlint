 ...
 */
Linter.prototype.applyRule = function applyRule(moddleRoot, ruleDefinition, fix) {
...
Linter.prototype.lint = function(moddleRoot, config) {
  ...
  return this.resolveRules(config).then((ruleDefinitions) => {
    ...
    ruleDefinitions.forEach((ruleDefinition) => {
      ...
      console.log('apply rule with config', config);

      const reports = this.applyRule(moddleRoot, ruleDefinition, config.fix);
