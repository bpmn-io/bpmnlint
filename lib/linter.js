...
Linter.prototype.resolveConfig = function(name) {
  ...
  return Promise.resolve(this.resolver.resolveConfig(pkg, configName)).then((config) => {
    ...
    const actualConfig = this.cachedConfigs[id] = prefix(config, pkg);
...
Linter.prototype.resolveConfiguredRules = function(config) {
  ...
  return Promise.all(
    ...
    const overrideRules = prefix(config, 'bpmnlint').rules;
...
Linter.prototype.lint = function(moddleRoot, config) {
  ...
  return this.resolveRules(config).then((ruleDefinitions) => {
    ...
    ruleDefinitions.forEach(({ rule, name, value }) => {
      ...
      finalReport[simpleRuleName(name)] = categorizedReports;
    });
...
function prefix(config, pkg) {

  if (pkg.indexOf('bpmnlint-plugin-') === 0) {
    pkg = pkg.substring('bpmnlint-plugin-'.length);
  }
...
function simpleRuleName(ruleName) {
  if (ruleName.indexOf('bpmnlint/') === 0) {
    return ruleName.substring('bpmnlint/'.length);
  }
