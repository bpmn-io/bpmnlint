...
const getDocumentationUrl = require('./utils/documentation');
...
Linter.prototype.applyRule = function applyRule(moddleRoot, ruleDefinition) {
  ...
  // Read documentation rule from rule metadata if exists. Otherwise get the url based on rule name
  let ruleMetaData = rule?.meta || {};
  const documentationUrl = ruleMetaData?.documentation?.url ? ruleMetaData?.documentation?.url : getDocumentationUrl(name);
  ruleMetaData.documentation = { url : documentationUrl };
