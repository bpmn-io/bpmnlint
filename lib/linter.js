 ...
 * @param {*} ruleFlagIdx
 */
function applyRule({ moddle, moddleRoot, ruleName, ruleFlagIdx }) {
  const rule = require(`../rules/${ruleName}`);
  const flagName = flagsMap[ruleFlagIdx];
  let reports = [];

  if (ruleFlagIdx) {
    reports = testRule({ moddle, moddleRoot, rule });
  }

  return { [flagName]: reports };
}

module.exports = async function linter({ moddle, moddleRoot, config }) {
  ...
  Object.entries(config).forEach(([ruleName, ruleFlagIdx]) => {
    const [flagName, reports] = Object.entries(
      applyRule({ moddle, moddleRoot, ruleName, ruleFlagIdx })
    )[0];
