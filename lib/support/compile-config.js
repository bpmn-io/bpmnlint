...
`;
  ...
  const ruleImportCode = Object.entries(rules).map(([ key, value ], idx) => {
  ...
  const moddleExtensionsImportCode = Object.entries(config.moddleExtensions || {}).map(([ key, importPath ], idx) => {

    if (typeof importPath !== 'string') {
      throw new Error(`illegal value for moddle-extension ${key}: expected <string>`);
    }
